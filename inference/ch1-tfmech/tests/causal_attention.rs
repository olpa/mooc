...
use ch1_tfmech::causal_attention;
...
fn run_case(fixture_name: &str) {
    ...
    let out = causal_attention(&q, &k, &v).expect("causal_attention failed");

    assert_close(&out, &fixture.out, 1e-4);
}

#[test]
fn causal_attention_small() {
    run_case("causal_attention_small");
}

#[test]
fn causal_attention_small_diff_v() {
    run_case("causal_attention_small_diff_v");
}

#[test]
fn causal_attention_fixed() {
    run_case("causal_attention_fixed");
}

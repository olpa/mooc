...
fn run_case(fixture_name: &str) {
    ...
    tensors.insert("w1.bias".to_string(), tensor1(&fixture.w1_bias));
    tensors.insert("w2.weight".to_string(), tensor2(&fixture.w2_weight));
    tensors.insert("w2.bias".to_string(), tensor1(&fixture.w2_bias));

    let mut vb = VarBuilder::from_tensors(tensors, DType::F32, &Device::Cpu);
    let ff = FeedForward::new(fixture.hidden_dim, fixture.intermediate_dim, &mut vb)
        .expect("failed to build FeedForward");

    let x = tensor3(&fixture.x);
    let out = ff.forward(&x).expect("forward failed");

    assert_close(&out, &fixture.out, 1e-4);
}

#[test]
fn feed_forward_small() {
    run_case("feed_forward_small");
}

...
use ch1_tfmech::MultiHeadAttention;
...
fn run_case(fixture_name: &str) {
    ...
    let mha = MultiHeadAttention::new(hidden_dim, fixture.num_heads, &mut vb)
        .expect("failed to build MultiHeadAttention");

    let x = tensor3(&fixture.x);
    let out = mha.forward(&x).expect("forward failed");

    assert_close(&out, &fixture.out, 1e-4);
}

#[test]
fn multihead_attention_small() {
    run_case("multihead_attention_small");
}
